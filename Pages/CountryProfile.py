...
def linePlot(df,c,width = False):
    ...
    if not df.empty:
        # print(df)
        fig = px.line(df,x="Year",y="Value",color_discrete_sequence =px.colors.qualitative.Antique,color = "Country",markers=True,symbol="Country") #line plot
        fig.add_hline(y=df["Baseline"].iloc[0], line_width=2, line_dash="dash", line_color="black") #Reference line
        if (df["UpDown"].iloc[0]=="Down" and (df["Baseline"].iloc[0]<100 | width)): #This block for amplifier and mitigator zone after checking the conditions.
...
def visualizeOp(df,country):
    ...
    #Displaying the flag of the selected country
    try:
    ...
    #Displaying Investment Priority
...
# t_fig.update_layout(margin = dict(l=10,r=10,t=10,b=10))
    ...
    # )
                ...
                if showhide: #If the checkbox on the sidebar is selected for trends
                    ...
                    for j in trendData["Indicator"].unique():
                        ...
                        if (j in ["Prevalence of undernourishment"]): #As this is expressed in terms of the average in 2015
