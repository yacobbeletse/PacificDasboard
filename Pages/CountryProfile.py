...
def coloredPlot(df,c1,capital,i,height =600):
    #Function for coloring the bargraph
    #df - dataframe with data
    #c1 - streamlit component to place the visualization
    #capital - title of the graph
    #i - column with the values
...
def traffic(df1,index = "Country",visType="Des",check="nice",present=pd.DataFrame()):
    #Function to color the bargraph with condition.
    #index, visType, check and present are optional parameters used to reuse the function.

    df1 = df1.replace({0:0.1}) ## 0 values changed by 0.1 to distinguish them from NA values
    # df=df.transpose()
    # c1.write(df)
    # df.index.name=None
...
def showPlot(df,index = "Country",visType="Des",indicator="nice",present=pd.DataFrame()):
    #Function to dispaly two side by side plots.
...
def visualizeOp(df,country):
    ...
    # present = data.sort_values("Year").drop_duplicates(["Country", "Indicator"], keep = "last")
    # print(present)
    try:
    ...
    for i in capitals:
        ...
        if not dff[dff["Country"]==country].empty:
            if i=="Food Systems Resilience Score":
                ...
                st.text('Score: {}'.format(presentValue))
