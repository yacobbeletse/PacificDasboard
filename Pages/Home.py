...
def app():
    ...
    info = {"red":"Worst","yellow":"Fine","green":"Excellent","gray":"Data Missing"}
    ...
    for j in ["Country","Availability","Accessibility", "Utilization","Stability"]:
        ...
        for i in df.columns:
            if i not in  [ "Bar", "About","Unit","Aspect","Pillar","Indicator", "Pacific"]:
                if j=='Country':
                    c[0].write("<h2 style='height: 279px;'>{}</h2>".format(i), unsafe_allow_html=True)
                    # html_string = ""
                    # try:
                    ...
                    df1 = df[df["Pillar"]==j]
                    df1["Status"] = df1[i].replace(info)
                    # print(df1)
                    # for i in df1.columns:
                        # if i not in  [ "Bar", "About","Unit","Aspect","Pillar","Indicator", "Pacific"]:
                            # print(df[i])
                    fig = px.bar_polar(df1, r = "Bar", theta="Indicator",color=i,color_discrete_map=color,hover_name="Indicator", 
                                       hover_data={
                                        'Status': True,
                                        'Bar':False,
                                        "Indicator": False,
