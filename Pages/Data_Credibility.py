...
def app():
    ...
    if choice=="Country":
        ...
        for i in pillars:
            ...
            if temp.empty:
                st.subheader("No data for {}".format(i))
            else:
                for k in temp["Aspect"].unique():
                    st.subheader(k)
                    data_fd = temp[temp["Aspect"]==k]
                    if data_fd.empty:
                        st.subheader("No data for {}".format(k))
                    else:
                        for j in data_fd["Indicator"].unique():
                            data_df = data_fd[data_fd["Indicator"]==j]
                            if data_df.empty:
                                st.subheader("No data for {}".format(j))
                            else:
                                st.write(j+', '+str(temp[temp["Indicator"]==j]["Year"].iloc[0]))

        
    else:
        pillar = st.sidebar.selectbox('Pillar',data_temp["Pillar"].unique())
        aspect = st.sidebar.selectbox('Aspect',data_temp["Aspect"].unique())
        indicator = st.sidebar.selectbox("Indicator", data_temp[(data_temp["Pillar"]==pillar) & (data_temp["Aspect"]==aspect)]["Indicator"].dropna().unique())
