...
disasterData = pd.read_csv('alldisaster.csv',encoding = 'unicode_escape').fillna(0)
...
def doSA(df,scale,shock,intensity,duration,c1,c2,country=None,con=None):
    global dataColl
    # year =  st.sidebar.slider('Enter the year for analysis', min_value=1900,max_value=2022,value=(2000,2022))
    # print(year,year[0],year[1])
    # df = disasterData[(disasterData['Year']>=year[0]) & (disasterData['Year']<=year[1])]
    # print(df.head())
    # disinfo = pd.read_csv('disInfo.csv',index_col ='index')
    disinfo = df.groupby(['Country','Disaster Type'])[['Total Deaths_new','Total Affected_new','AdjustedDamages_new']].sum().apply(np.ceil).reset_index()
    disinfo = disinfo.assign(intensity=lambda x: x[['Total Deaths_new','Total Affected_new','AdjustedDamages_new']].mean(axis=1))
    disinfo = pd.pivot_table(disinfo[['Country', 'Disaster Type','intensity']],values='intensity',index='Country',columns=['Disaster Type']).fillna(0)
    # disinfo = disinfo.rename(columns = {'intensity':'intensities'})
    print(disinfo.head())
    # print('Printing shock: '+shock)
    # print(disinfo['Disaster Type'].unique())
    # disinfo = disinfo[disinfo['Disaster Type']==shock]
    yearChoice = 2020
    if scale=="Global":
        # temp = pd.DataFrame(disinfo[shock])
        temp = disinfo.copy()
        temp[shock]=temp[shock].apply(np.round)
        print(temp.head())
        ...
        disdata[country]=disdata[country].apply(np.round)
        # print(disdata.head())
        # fig3 = px.bar(disdata.sort_values(by=country,ascending=False), y =country , x = 'index',orientation='v',text = country)
        # fig3 = px.bar(disdata.sort_values(by=country,ascending=False), y =country , x = 'Disaster Type',orientation='v',text = country)
        ...
        try:
            # fig3 = px.bar(disdata.sort_values(by=country,ascending=False), y =country , x = 'Disaster Type',orientation='v',text = country)
            fig3 = px.bar(disdata.sort_values(by=country,ascending=False), x =country , y = 'Disaster Type',orientation='h',text = country)
            # fig3.update_layout(yaxis_range=[0,max(disdata[country])+5],yaxis_title='Standardized Impact Score',xaxis_title=None, font = dict(
            #     size =18,
            # )
            # )
            fig3.update_layout(xaxis_range=[0,max(disdata[country])+5],xaxis_title='Standardized Impact Score',yaxis_title=None, font = dict(
                size =18,
            )
            )
            fig3.update_xaxes(tickfont=dict(size =15, family = "Arial Black"))
            ...
            con.plotly_chart(fig3)
            # disdata = disinfo[disinfo.index==country].transpose().reset_index()
            # disdata = disinfo[disinfo.index==country]
            # print(disdata.head())
...
if(analysisType=="World Map"):
    ...
    doSA(df,scale,shock,intensity_score,1,c1,c2,country,con=conPlots)
